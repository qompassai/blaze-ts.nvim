...
function M.detect_gpu()
	...
	local success, nvidia_smi =
		pcall(io.popen, "nvidia-smi --query-gpu=count,name,driver_version --format=csv,noheader 2>/dev/null")
...
function M.health_check()
	...
	if env.gpu.has_nvidia then
		table.insert(
			health.features,
			"NVIDIA GPU detected" .. (env.gpu.cuda_version and " (CUDA " .. env.gpu.cuda_version .. ")" or "")
		)
	end
	...
	if env.gpu.has_amd then
		table.insert(
			health.features,
			"AMD GPU detected" .. (env.gpu.rocm_version and " (ROCm " .. env.gpu.rocm_version .. ")" or "")
		)
	end
